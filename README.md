...
- 清理气泡、字体、挂件、VIP/群荣誉及扩展昵称、名片、头衔等元数据
- 移除扩展消息 ID，替换上传记录的 `msgRandom`；引用和嵌套副本使用一致的合成索引
...
媒体元素目前按原始数据保留，因此原始文件哈希和 QQ 媒体资源标识仍可能关联到原消息。强匿名模式需要增加“下载后重新上传媒体”的流程（不过个人认为这些内容需要拥有QQ内部权限的人才能倒查出来源，所以不影响使用）。
